        ...
        <div class="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="flex items-center gap-3">
                <a href="{{ route('vendor.discounts') }}"
                    class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white border border-zinc-200 text-zinc-600 transition hover:bg-zinc-50 hover:text-zinc-900">
            ...
            <template x-if="itemId">
                <button type="button" @click="itemId = ''; step = 1; name = ''; value = '';"
                    class="inline-flex items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-xs font-bold text-zinc-600 hover:bg-yellow-100 transition shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        <div class="flex flex-wrap items-center gap-6">
                            ...
                            <button type="button" @click="step = 1"
                                ...
                                <div>
                                    <span class="block text-xs font-extrabold uppercase tracking-wider text-zinc-400">Step
                                        1</span>
                                    <span class="text-sm font-bold transition-colors"
                                        :class="step === 1 ? 'text-zinc-900' : 'text-zinc-500 group-hover:text-zinc-800'">Select
                                        Product</span>
                                </div>
                            </button>
                            ...
                            <!-- Step 2 Button -->
                            <button type="button" @click="if(itemId) step = 2" :disabled="!itemId"
                                ...
                                <div>
                                    <span class="block text-xs font-extrabold uppercase tracking-wider text-zinc-400">Step
                                        2</span>
                                    <span class="text-sm font-bold transition-colors"
                                        :class="step === 2 ? 'text-zinc-900' : 'text-zinc-500 group-hover:text-zinc-800'">Campaign
                                        Details</span>
                                </div>
                            </button>
                            ...
                            <!-- Step 3 Button -->
                            <button type="button" @click="if(itemId && name && value) step = 3"
                                ...
                                <div>
                                    <span class="block text-xs font-extrabold uppercase tracking-wider text-zinc-400">Step
                                        3</span>
                                    <span class="text-sm font-bold transition-colors"
                                        :class="step === 3 ? 'text-zinc-900' : 'text-zinc-500 group-hover:text-zinc-800'">Schedule
                                        & Limits</span>
                                </div>
                            </button>
                        </div>
                ...
                <form action="{{ route('vendor.discounts.store') }}" method="POST" class="space-y-8">
                    ...
                    <div x-show="step === 1" x-transition
                        class="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm space-y-6">
                        <div class="flex items-center justify-between border-b border-zinc-100 pb-4">
                            ...
                            <span class="text-xs text-zinc-400 font-semibold"
                                x-text="Object.keys(itemsList).length + ' products available'"></span>
                        </div>
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" x-model="productSearch" placeholder="Filter products by name..."
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[380px] overflow-y-auto pr-2">
                            <template x-for="item in filteredProducts" :key="item.item_id">
                                <div @click="itemId = item.item_id; step = 2;"
                                    ...
                                    <div
                                        class="h-12 w-12 rounded-xl overflow-hidden bg-zinc-50 border border-zinc-100 flex items-center justify-center shrink-0">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-zinc-400 mt-0.5">Price: ₱<span
                                                x-text="item.price.toFixed(2)"></span> · Stock: <span
                                                x-text="item.stock"></span></p>
                                    </div>
                        ...
                        <div class="pt-4 border-t border-zinc-100 flex justify-end">
                            <button type="button" @click="step = 2" :disabled="!itemId"
                    ...
                    <div x-show="step === 2" x-transition
                        ...
                        <template x-if="currentItem">
                            <div
                                class="flex items-center justify-between bg-zinc-50 border border-zinc-200 rounded-2xl p-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="h-10 w-10 rounded-xl overflow-hidden bg-white border shrink-0 flex items-center justify-center">
                                    ...
                                    <div>
                                        <span class="block text-xs font-bold text-zinc-400 uppercase tracking-wider">Target
                                            Product</span>
                                        <span class="text-sm font-extrabold text-zinc-900" x-text="currentItem.name"></span>
                                    </div>
                                </div>
                                <button type="button" @click="step = 1"
                                    class="text-xs font-bold text-emerald-600 hover:text-emerald-700">Change</button>
                            </div>
                        </template>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                class="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3.5 text-zinc-800 outline-none transition-all duration-200 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 font-medium">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs text-red-600 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="pt-4 border-t border-zinc-100 flex items-center justify-between gap-4">
                            ...
                            <button type="button" @click="step = 3" :disabled="!name || !value || parseFloat(value) <= 0"
                                class="rounded-2xl bg-zinc-900 px-6 py-3.5 text-sm font-bold text-white transition hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 shadow-sm">
                                Next: Schedule
                                <x-heroicon-o-arrow-right class="h-4 w-4" />
                            </button>
                    ...
                    <div x-show="step === 3" x-transition
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" id="date_start" name="date_start" x-model="dateStart"
                            ...
                            <div>
                                ...
                                <input type="datetime-local" id="date_end" name="date_end" x-model="dateEnd"
                                    class="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3.5 text-zinc-800 outline-none transition-all duration-200 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 font-medium"
                                    required>
                                @error('date_end')
                                    <p class="mt-1.5 text-xs text-red-600 font-semibold">{{ $message }}</p>
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="number" id="use_limit" name="use_limit" value="{{ old('use_limit') }}"
                            ...
                            <div class="flex flex-col justify-end">
                                <label
                                    class="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 transition-all duration-200 hover:border-emerald-500/30 hover:bg-emerald-50/10 cursor-pointer h-[54px]">
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-6 space-y-6">
                    ...
                    <div
                        class="rounded-3xl border border-zinc-200 bg-white shadow-xl overflow-hidden p-6 space-y-6 transition-all duration-300">
                        <div
                            class="relative aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-zinc-50 to-zinc-100 border border-zinc-100 flex items-center justify-center">
                            {{-- Image Preview --}}
                            ...
                            <div class="absolute top-4 left-4 flex flex-col gap-2 z-10">
                                ...
                                <template x-if="value > 0">
                                    <div
                                        class="bg-red-600 text-white text-[10px] font-black px-3 py-1.5 rounded-full shadow-md tracking-wider">
                                        <span x-show="type === 'percentage'"
                                            x-text="parseFloat(value).toFixed(0) + '% OFF'"></span>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">STOREFRONT
                                    ITEM</span>
                                <template x-if="isActive">
                                    <span
                                        class="inline-flex items-center gap-1 rounded bg-emerald-50 border border-emerald-100 px-2 py-0.5 text-[9px] font-bold text-emerald-700 uppercase tracking-wider">Live</span>
                                </template>
                            </div>

                            {{-- Product Name --}}
                            <h4 class="text-lg font-extrabold text-zinc-900 leading-tight truncate"
                                x-text="currentItem ? currentItem.name : 'Choose a product...'"></h4>

                            {{-- Campaign Title --}}
                            <template x-if="name.length > 0">
                                <div
                                    class="bg-emerald-50/50 border border-emerald-100/50 rounded-xl p-2.5 flex items-center gap-2">
                                ...
                                <template x-if="currentItem && value > 0">
                                    <div class="flex items-baseline gap-2">
                                        <span class="text-2xl font-black text-green-600"
                                            x-text="'₱' + parseFloat(promoPrice).toFixed(2)"></span>
                                        <span class="text-sm font-semibold text-zinc-400 line-through"
                                            x-text="'₱' + parseFloat(currentItem.price).toFixed(2)"></span>
                                    </div>
                                </template>
                                <template x-if="!currentItem || value <= 0">
                                    <span class="text-2xl font-black text-zinc-900"
                                        x-text="currentItem ? '₱' + parseFloat(currentItem.price).toFixed(2) : '₱0.00'"></span>
                                </template>
                            </div>
                        </div>
                    </div>
