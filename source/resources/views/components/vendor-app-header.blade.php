<header class="flex h-15 items-center justify-between border-b border-gray-200 bg-white px-8">
    ...
    <nav class="flex items-center gap-1">
        ...
        <a href="{{ route('vendor.discounts') }}"
            class="{{ request()->routeIs('vendor.discounts*') || request()->path() === 'vendor-discounts' ? 'border border-green-200 bg-green-50 text-green-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900' }} flex items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-sm transition-colors">
        ...
        <div class="relative group">
            ...
            {{-- Dropdown Menu --}}
            <div
                class="absolute right-0 mt-2 w-48 bg-white rounded-lg border border-gray-200 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                <a href="/vendor-profile"
                    class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 first:rounded-t-lg">
                ...
                <form method="POST" action="/logout" class="block">
                    @csrf
                    <button type="submit"
                        class="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-600 hover:text-white transition-colors duration-150 last:rounded-b-lg border-t border-gray-200">
